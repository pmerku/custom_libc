...
void test_construct() {
	...
	two.insert(26);
	two.insert(42);
	ft::set<int> three(two.begin(), two.end());
	ft::set<int> four(three);
	ft::set<int> five = four;
...
void test_iterators() {
	...
	ft::set<int>::const_iterator ftcit(ftit), ftcite(ftite);
	auto ftrit = ftSet.rbegin(), ftrite = ftSet.rend();
	ft::set<int>::const_reverse_iterator ftcrit(ftrit), ftcrite(ftrite);

	auto stdit = stdSet.begin(), stdite = stdSet.end();
	std::set<int>::const_iterator stdcit(stdit), stdcite(stdite);
	auto stdrit = stdSet.rbegin(), stdrite = stdSet.rend();
	std::set<int>::const_reverse_iterator stdcrit(stdrit), stdcrite(stdrite);
...
void test_modifiers() {
	...
	}
	{
		...
		ASSERT_THROW(ftSet.size(), size_t(1));
...
void test_lookup() {
	...
	ft::pair<ft::set<int>::iterator, ft::set<int>::iterator> ftPair = ftSet.equal_range(35);
