...
static unsigned long long	spec_unsigned_int_read_val(t_ft_printf *pf, t_ft_fmt *fmt) {
	va_list *args = pf->args;
	if (fmt->length == hh) {
...
static unsigned long long	read_into_buf(t_ft_printf *pf, t_ft_fmt *fmt, char *buf) {
	...
	ft_memset(buf, '\0', 130);
	unsigned long long i = spec_unsigned_int_read_val(pf, fmt);
...
static void					append_nbr_prefix(t_ft_printf *pf, t_ft_fmt *fmt, int nbr_space, unsigned long long i) {
	...
	if (fmt->leftpad_zeroes == 1 && fmt->precision_set == 0 && fmt->left_justify == 0 && nbr_space > 0 && fmt->hashtag == 1) {
		buf_putchars(pf->buf, '0', nbr_space);
	}
}
...
void						spec_base(t_ft_printf *pf, t_ft_fmt *fmt) {
	...
	unsigned long long i = read_into_buf(pf, fmt, buf);
