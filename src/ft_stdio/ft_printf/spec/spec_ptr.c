...
static unsigned long long	read_into_buf(t_ft_printf *pf, char *buf, t_ft_fmt *fmt) {
	ft_memset(buf, '\0', 130);
	unsigned long long i = spec_unsigned_int_read_val(pf);
...
static void					append_nbr_prefix(t_ft_printf *pf, t_ft_fmt *fmt, int nbr_space) {
	...
	if (fmt->leftpad_zeroes == 1 && fmt->precision_set == 0 && fmt->left_justify == 0 && nbr_space > 0) {
